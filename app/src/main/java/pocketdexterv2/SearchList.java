...
/*
 * This is where the search is executed and displayed to the user.
 */
public class SearchList extends Activity implements OnItemClickListener {
	...
	private static ArrayList<Pokemon> list = new ArrayList<Pokemon>();
	...
	public void onItemClick(AdapterView<?> l, View v, int position, long id) {
        ...
        intent.putExtra("mon", list.get(position));
	...
	private void assignBackground() {
		...
		RelativeLayout layout = (RelativeLayout) findViewById(R.id.searchlist);
	...
	//This method sets the Pokedex from the options and executes a search based on a user's criteria.
	private void applySort() {
        ...
        String info = intent.getStringExtra(Search.EXTRA_MESSAGE);
		...
		search(master, info, prefs);
		sortList(list, prefs);

		//Converts the list of Pokemon into a list of formatted Strings
		for(int i=0; i<list.size(); i++) {
			master2.add(formatEntry(prefs, list.get(i)));
		}
	...
	//This method scans an ArrayList of Pokemon(array) using the user's search criteria(choice).
	private void search(ArrayList<Pokemon> array, String choice, SharedPreferences prefs) {
		...
		if(choice != null) {
			...
			if(scan.hasNextInt()) {
				place = scan.nextInt();
				if(place >= 1 && place <= array.size()) {
					pick = array.get(place-1);
					list.add(pick);
					return;
					
				} else if(place > array.get(0).getNum() && place < array.get(array.size()-1).getNum()) {
					int i = 1;
					while(i <= array.size()) {
						pick = array.get(i-1);
						if(place == pick.getNum()) {
							list.add(pick);
							return;
						}
						i++;
				...
				while(i <= array.size()) {
					place = i;
					pick = array.get(i-1);
					if(pick.getName().toLowerCase(Locale.US).contains(name.toLowerCase(Locale.US))) {
						list.add(pick);
					}
					//checks whether to get alternate forms
					if(prefs.getBoolean("search_alts_checkbox", false)) {
						for(int j=0; j<pick.getAlts().size(); j++) {
							if(pick.getAlts().get(j).getName().toLowerCase(Locale.US).contains(name.toLowerCase(Locale.US))) {
								list.add(pick.getAlts().get(j));
							}
						}
					}
					i++;
			...
			int i = 1;
			while(i <= array.size() && !type1.equals("none") && !type2.equals("none")) {
				place = i;
				pick = array.get(i-1);
				if((type1.equalsIgnoreCase(pick.getType()) && type2.equalsIgnoreCase(pick.getType2())) || (type2.equalsIgnoreCase(pick.getType()) && type1.equalsIgnoreCase(pick.getType2()))) {
					list.add(pick);
				} else if((type1.equalsIgnoreCase(pick.getType()) || type1.equalsIgnoreCase(pick.getType2())) && type2.equals("---------")) {
					list.add(pick);
				} else if(type1.equalsIgnoreCase(pick.getType()) && pick.getType2() == null && type2.equals(type1)) {
					list.add(pick);
				}
				...
				if(prefs.getBoolean("search_alts_checkbox", false)) {
					for(int j=0; j<pick.getAlts().size(); j++) {
						if((type1.equalsIgnoreCase(pick.getAlts().get(j).getType()) && type2.equalsIgnoreCase(pick.getAlts().get(j).getType2())) || (type2.equalsIgnoreCase(pick.getAlts().get(j).getType()) && type1.equalsIgnoreCase(pick.getAlts().get(j).getType2()))) {
							list.add(pick.getAlts().get(j));
						} else if((type1.equalsIgnoreCase(pick.getAlts().get(j).getType()) || type1.equalsIgnoreCase(pick.getAlts().get(j).getType2())) && type2.equals("---------")) {
							list.add(pick.getAlts().get(j));
						} else if(type1.equalsIgnoreCase(pick.getAlts().get(j).getType()) && pick.getAlts().get(j).getType2() == null && type2.equals(type1)) {
							list.add(pick.getAlts().get(j));
						}
					}
				}
			...
			if(list.size() == 0 && type1.equals("none") && type2.equals("none")) {
				text.setText("Search yielded no results for \"" + name + "\"");
			} else if(list.size() == 0) {
				text.setText("Search yielded no results for \"" + type1 + "/" + type2 + "\"");
			} else {
				text.setText("");
			}
		}
		...
		switch (sort) {
			case "0":
				if (!descend) {
					...
					Collections.sort(list, new ZAComparator());
				}
				break;
			case "1":
				if (!descend) {
					...
					Collections.sort(list, new Num2Comparator());
				}
				break;
			case "2":
				if (!descend) {
					Collections.sort(list, new HighStatsComparator());
				} else {
					Collections.sort(list, new LowStatsComparator());
				}
				break;
			case "3":
				if (!descend) {
					Collections.sort(list, new HighHpComparator());
				} else {
					Collections.sort(list, new LowHpComparator());
				}
				break;
			case "4":
				if (!descend) {
					Collections.sort(list, new HighAttackComparator());
				} else {
					Collections.sort(list, new LowAttackComparator());
				}
				break;
			case "5":
				if (!descend) {
					Collections.sort(list, new HighDefenseComparator());
				} else {
					Collections.sort(list, new LowDefenseComparator());
				}
				break;
			case "6":
				if (!descend) {
					Collections.sort(list, new HighSpatkComparator());
				} else {
					Collections.sort(list, new LowSpatkComparator());
				}
				break;
			case "7":
				if (!descend) {
					Collections.sort(list, new HighSpdefComparator());
				} else {
					Collections.sort(list, new LowSpdefComparator());
				}
				break;
			case "8":
				if (!descend) {
					Collections.sort(list, new HighSpeedComparator());
				} else {
					Collections.sort(list, new LowSpeedComparator());
				}
				break;
		}
	}
	...
	private static void RSE() {
		...
		master.add(new Pokemon(299, "Nosepass", "Rock", null, "Level up at Location", 375, 30, 45, 135, 45, 90, 30, false, false));
